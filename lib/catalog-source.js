...
// hieronder aan — nooit rechtstreeks Shopify of de placeholder-data. Welke
// bron daadwerkelijk gebruikt wordt, hangt af van de environment variables:
//
...
//   3. Niets van het bovenstaande                   -> lib/legacy-placeholder-products.js
//
...
// toont automatisch de echte producten.
//
...
//
// GEÏMPORTEERDE LEVERANCIERSCATALOGUS (bv. 1on1 Wholesale-CSV's, zie
// scripts/import-supplier-products.js en IMPORT-GUIDE.md): dit is een
// EXTRA bron, TUSSEN Shopify en de handmatige placeholder-data in — gebruikt
// zodra 'ie bestaat, maar verandert NIETS aan de Shopify-paden hierboven.
// Zodra je wél echte Shopify-tokens instelt, wint Shopify automatisch weer.

const { fetchProducts } = require('./shopify/storefront-client');
const { LEGACY_PLACEHOLDER_PRODUCTS } = require('./legacy-placeholder-products');
...
function getImportedSupplierProducts() {
  try {
    // Alleen laden als het bestand daadwerkelijk bestaat (na een import-run) —
    // vóór de eerste import bestaat dit bestand nog niet, en dat is prima.
    return require('./supplier-import-products').SUPPLIER_IMPORT_PRODUCTS;
  } catch (e) {
    return null;
  }
}
...
function normalizeShopifyProduct(node, index) {
  ...
  return {
    ...
    // andere Shopify-variant-ID mapt is nog niet gebouwd (zie RC1/PRODUCTION-READINESS).
...
async function getCatalog() {
  ...
  const imported = getImportedSupplierProducts();
  if (imported && imported.length) {
    return imported;
  }
  // Nog geen Shopify (test of echt) EN geen geïmporteerde leverancierscatalogus
  // -> terugvallen op de handmatige placeholders zodat de site tijdens de
  // vroegste ontwikkelfase nog steeds bruikbaar is.
  return LEGACY_PLACEHOLDER_PRODUCTS;
}
...
function getCatalogSourceLabel() {
  ...
  const imported = getImportedSupplierProducts();
  if (imported && imported.length) return 'Geïmporteerde leverancierscatalogus (' + imported.length + ' producten)';
  return 'Placeholder-data (nog geen Shopify geconfigureerd, nog geen import gedraaid)';
}
