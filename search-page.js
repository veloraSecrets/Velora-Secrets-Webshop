...
(() => {
  ...
  function renderNoResults() {
    ...
    const suggestions = window.VELORA_PRODUCTS.slice().sort((a, b) => b.rating - a.rating).slice(0, 4);
  ...
  function getActiveFilters() {
    ...
    const minRatings = [...document.querySelectorAll('[data-rating-filter]:checked')].map((c) => Number(c.dataset.ratingFilter));
    const onlyInStock = document.querySelector('[data-availability-filter="in-stock"]')?.checked;
    return { categories, brands, priceRanges, minRatings, onlyInStock };
  }

  function applyFiltersAndSort(baseResults) {
    const { categories, brands, priceRanges, minRatings, onlyInStock } = getActiveFilters();
    const minRating = minRatings.length ? Math.max(...minRatings) : 0;
    ...
    let results = baseResults.filter((p) => {
      ...
      return matchesCategory && matchesBrand && matchesPrice && matchesRating && matchesAvailability;
    });
  ...
  if (!query.trim()) {
    ...
    if (!baseResults.length) {
      ...
      document.querySelectorAll('[data-category-filter], [data-brand-filter], [data-price-filter], [data-rating-filter], [data-availability-filter]').forEach((el) =>
